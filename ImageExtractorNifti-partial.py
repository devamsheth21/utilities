...
import pdb 
configs = {}
def remaining_volumes(volume_list):
    # Load the mapping files
    df_mapping = pd.read_csv('/media/Datacenter_storage/chestCT_birads/data/mapping_PEdata.csv')

    # Remove duplicates based on the 'dicom' column
    # df_mapping = df_mapping.drop_duplicates(subset=['dicoms'],keep='first') # kept the first ones in duplicates for the chestCT data
    df_mapping = df_mapping.drop_duplicates(subset=['dicoms'],keep=False)


    # Remove entries where the 'dicom' path does not exist
    df_mapping = df_mapping[df_mapping['niftis'].apply(lambda x: os.path.exists(x))]
    df_mapping.to_csv('/media/Datacenter_storage/chestCT_birads/data/mapping_PEdata_final.csv',index=False)
    # # Get the set of dicom paths from the combined dataframe
    # processed_volumes = set(df_mapping['dicom'].apply(lambda x: os.path.split(x)[0]))
    volumes_existing = set(df_mapping['dicoms'].unique())
    # Subtract the processed volumes from the original volume list
    volume_list = list(set(volume_list) - volumes_existing)
    return volume_list
...
def initialize_config_and_execute(config_values):
    ...
    logging.info("This extraction is being done with following script : "+ __file__)
...
def get_tuples(plan, outlist = None, key = ""):
    ...
    for aa  in plan.dir():
        try:
            ...
            logging.warning(f'Error {aa} is not supported')
            continue 
...
def extract_images(input_tup):
    (filedata, i, nifti_destination, flattened_to_level, failed, is16Bit) = input_tup
    ...
    try:
        if flattened_to_level == 'patient':
            ...
            if flattened_to_level !='anon': 
                ...
                folderName = hashlib.sha224(filedata['file'].iloc[i].encode('utf-8')).hexdigest()
                niftifile = nifti_destination+folderName + '/' + 'vol'+ '.nii.gz'
            # check for existence of the folder tree patient/study/series. Create if it does not exist.
            os.makedirs(nifti_destination + folderName,exist_ok=True)
...
def gen_par_inputs(filedata,nifti_destination,flattened_to_level,failed,is16Bit):
    for i in range(len(filedata)):
        yield (filedata, i, nifti_destination, flattened_to_level, failed, is16Bit)
...
def execute(pickle_file, dicom_home, output_directory, print_images, print_only_common_headers, depth,
    ...
    file_path = dicom_home  
    if os.path.isfile(pickle_file):
        with open(pickle_file,'rb') as f: 
            filelist=pickle.load(f)
        volume_list = set([os.path.split(e)[0] for e in filelist])
        volume_list = list(volume_list)
    else:
        filelist = [str(e) for e in Path(dicom_home).rglob('*.dcm') ] # search the folders at the depth we request and finds all dicoms
        pickle.dump(filelist,open(pickle_file,'wb'))
        #get all the patient folders
        patient_dict = {}
        volume_list  =[]
        volume_list  =  filelist 
        volume_list = set([os.path.split(e)[0] for e in volume_list])
        volume_list = list(volume_list)
    
    logging.info('Number of dicom files: ' + str(len(filelist)))
    logging.info('Number of volumes: ' + str(len(volume_list)))
    # Call remaining_volumes to reduce the volume list
    volume_list = remaining_volumes(volume_list)
    logging.info(f"Number of volumes after reduction: {len(volume_list)}")
    ...
    for i,chunk in enumerate(file_chunks):
        ...
        if os.path.isfile(mappings):
            print(f"skiping {csv_destination}")
            continue 
        fm = open(mappings, "w+")
        ...
        with Pool(core_count) as p:
            ...
            if len(headerlist) >= chunk_size:
                ...
                data.to_csv(csv_destination, mode='a', index=False, header=not os.path.exists(csv_destination))
                headerlist = []  # Reset the list to free memory

        # Process any remaining headers
        if headerlist:
            data = pd.DataFrame(headerlist)
            logging.info('Final Chunk Number of fields per file : ' + str(len(data.columns)))
            data.to_csv(csv_destination, mode='a', index=False, header=not os.path.exists(csv_destination))
        ...
        if print_images:
            ...
            pokemon = open('fail_log-chestct.txt','w')
            filedata = data
            total = len(chunk)
            stamp = time.time()
            par_inputs = gen_par_inputs(filedata,nifti_destination,flattened_to_level,failed,is16Bit)
            with Pool(core_count) as p: 
                res = p.imap_unordered(extract_images,par_inputs)
                for i,outs in enumerate(res): 
                    (fmap,fail_path,err) = outs 
                    if err:
                        count +=1
...
if __name__ == "__main__":
    ...
    niffler = {
    # "DICOMHome":"/home/gokul/Datacenter_storage/gokul/self-supervision/data/NIFTI/DICOMPull/exported_dicom/",
    # "DICOMHome": "/media/Datacenter_storage/Bhavika_radiogenomics/Data/Images/Arizona_anno/2EBOTZHRXXT4JDABE7M2SWU8P/1.2.832.0.1.3680043.2.783.307.4.129110567.138684423933410.1/1.2.840.113619.2.416.6574348.4028401.18480.1519045031.150",
    "DICOMHome": "/media/Datacenter_storage/Bladder_project/Bladder Study_first_pull/Bladder-StudyDownload-2021-10-21STUTZMAN_DAVID__MR_/CT_ABDOMEN_PELVIS_WITHOUT_IV_CONTRAST-2018-10-22",
    "OutputDirectory": "/media/Datacenter_storage/Devam/CT/niftitest",
    "Depth": 2,
    "SplitIntoChunks": 100,
    "PrintImages": True,
    "CommonHeadersOnly": False,
    "UseProcesses": 0 ,
    "FlattenedToLevel": "others",
    "is16Bit":True,
    "SendEmail": False ,
    ...
    ap.add_argument("--YourEmail", default=niffler['YourEmail'])
     # Custom help message with an example command
    argparse.epilog = 'Example command:\n  python  ImageExtractorNifti.py --DICOMHome "/media/Datacenter_storage/MRI_rectal_cancer/data/Dicoms" --OutputDirectory "/media/Datacenter_storage/MRI_rectal_cancer/data/" --depth 2'
